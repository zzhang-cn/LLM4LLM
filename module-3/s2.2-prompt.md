#### The Concept of "Dark Data"

A key insight behind test-time computation is leveraging what we can call "dark data":

1. **What is Dark Data?**
   - The rich internal reasoning processes humans use when solving problems
   - These processes are rarely captured in training data
   - They include false starts, dead ends, verification steps, and revisions
...
#### Mathematical Formulation of Dark Data (For the Curious)

We can formalize the concept of dark data with a rigorous mathematical framework:

Let's define a reasoning problem as finding the right function $f$ that maps an input $x$ to the correct output $y$, i.e., $f(x) = y$.

Standard supervised learning optimizes:
$\min_\theta \mathbb{E}_{(x,y) \sim \mathcal{D}} \left[ \mathcal{L}(f_\theta(x), y) \right]$

Where $\mathcal{D}$ is the distribution of input-output pairs, $\theta$ represents model parameters, and $\mathcal{L}$ is a loss function.

However, for complex reasoning, humans typically decompose the problem into steps:
$y = f_n(f_{n-1}(...f_1(x)))$

The intermediate steps $(z_1, z_2, ..., z_{n-1})$ where $z_i = f_i(z_{i-1})$ (with $z_0 = x$) constitute the "dark data" - the internal reasoning process.

In traditional# Session 3.2: Beyond Pattern Matching to Reasoning: Test-Time Computation
