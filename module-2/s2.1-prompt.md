...
Transformers process all tokens in parallel (unlike RNNs that process one token at a time), so they need an explicit way to preserve position information.
...
**Understanding Check ✓**
- Why do transformers need explicit position information when RNNs don't?
- How do position embeddings help the model distinguish between sentences with the same words in different orders?
