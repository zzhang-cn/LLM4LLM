...
Let's implement a simple embedding lookup and see how it works:
...
tokens, embeddings = get_sentence_embeddings(sentence, embedding, word_to_idx)

# Print the tokens and their embeddings
print("Tokens and their embeddings:")
for token, embed in zip(tokens, embeddings):
    print(f"{token}: {embed.detach().numpy()}")
...
for i, token in enumerate(tokens):
    embed = embeddings[i].detach().numpy()
    ax.scatter(embed[0], embed[1], embed[2], marker='o')
    ax.text(embed[0], embed[1], embed[2], token)
...
# Example of processing a sequence with a neural network
...
model = SimpleLanguageModel(vocab_size, embedding_dim=3, hidden_dim=5, output_dim=vocab_size)

# Convert sentence to indices
indices_tensor = torch.tensor([word_to_idx.get(word, 0) for word in tokens])

# Get predictions
predictions = model(indices_tensor)
...
Formally, word embeddings can be represented as a matrix E ∈ ℝ^(V×D), where V is the vocabulary size and D is the embedding dimension. For a given word with index i, its embedding vector is the i-th row of E, denoted as E_i ∈ ℝ^D.

The embedding lookup operation can be expressed as:

$$e_i = E[i]$$

Where e_i is the embedding vector for the word with index i.

In neural language models, these embeddings are learned during training through backpropagation. The training objective encourages similar words to have similar embeddings, effectively placing them close together in the embedding space.

#### Joint Learning of Words and Weights

A key advantage of neural language models is that the embeddings (E) and the network weights (W) are learned jointly. This allows the model to develop specialized representations that work well for the specific task, rather than using fixed representations.

The learning process can be understood as optimizing an objective function J with respect to both E and W:

$$\min_{E,W} J(E, W)$$

Where J measures how well the model predicts the next word given the previous words. This joint optimization allows the embeddings to capture not just general semantic relationships, but also task-specific information relevant to language modeling.
