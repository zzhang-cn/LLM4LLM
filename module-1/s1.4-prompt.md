...
- Neural language models consistently outperform n-gram models on standard benchmarks
- They achieve lower perplexity (a measure of prediction quality)
- They make more sensible predictions for unseen contexts
- They scale better with more data and larger models

## Hands-On Implementation (For CS Students)

Let's implement an n-gram model and compare it directly with our neural language model trained on the same Brown corpus data:
...
class NgramLanguageModel:
    ...
    def calculate_perplexity(self, test_sentences):
        ...
        # Calculate perplexity
        perplexity = 2 ** (-log_likelihood / token_count)
        return perplexity
...
# Now let's compare with our neural language model
# We'll reuse the BengioLanguageModel and training code from KP3
...
### Specific Advantages Demonstrated:

1. **Handling Unseen Combinations**:
   The code comparison demonstrates how neural models can make reasonable predictions for word combinations not explicitly seen during training, while n-gram models resort to a basic smoothing approach that doesn't capture meaningful relationships.

2. **Parameter Efficiency**:
   Let's calculate the number of parameters for both models:

```python
# Calculate parameter counts for both models
vocab_size = len(vocab)

# N-gram model parameters (assuming a trigram model)
context_count = len(ngram_model.context_counts)
ngram_param_count = context_count * vocab_size

# Neural model parameters
embedding_params = vocab_size * embedding_dim
hidden_params = (context_size * embedding_dim) * hidden_dim + hidden_dim  # weights + biases
output_params = hidden_dim * vocab_size + vocab_size  # weights + biases
neural_param_count = embedding_params + hidden_params + output_params

print("\nParameter Efficiency:")
print(f"N-gram model parameters: {ngram_param_count:,}")
print(f"Neural model parameters: {neural_param_count:,}")
print(f"Ratio: {ngram_param_count / neural_param_count:.1f}x more parameters in n-gram model")
```

3. **Domain Adaptation**:
   Let's demonstrate how neural models can adapt to new domains more effectively:

```python
# Train on a different domain
train_sentences_fiction, test_sentences_fiction, _ = prepare_corpus_data(
    categories=['fiction'], context_size=context_size, min_freq=5)

# Train a new n-gram model on fiction
ngram_model_fiction = NgramLanguageModel(n=context_size+1, smoothing=True)
ngram_model_fiction.train(train_sentences_fiction)

# Test cross-domain performance
ngram_cross_perplexity = ngram_model.calculate_perplexity(test_sentences_fiction)
neural_cross_perplexity = calculate_neural_perplexity(model, test_sentences_fiction, word_to_idx, context_size)

print("\nCross-Domain Adaptation (News → Fiction):")
print(f"N-gram model perplexity: {ngram_cross_perplexity:.2f}")
print(f"Neural model perplexity: {neural_cross_perplexity:.2f}")
print(f"Difference: {(ngram_cross_perplexity - neural_cross_perplexity):.2f}")
```
